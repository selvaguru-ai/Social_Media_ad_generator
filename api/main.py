...
class RunRequest(BaseModel):
    ...
    resume: bool = False
    until: Optional[str] = "prospect"
    full: bool = False


class VerifyRequest(BaseModel):
    status: str = Field(description="quiet, has_ads, skip, or pending")
    ads_count_ui: Optional[int] = Field(default=None, ge=0)
    note: str = Field(default="", max_length=2000)


def _public_ad(ad: Dict[str, Any]) -> Dict[str, Any]:
    """Drop snapshot URLs — they can embed a Meta access token."""
    safe = {key: value for key, value in ad.items() if key != "ad_snapshot_url"}
    return safe
    ...
    return {
        ...
        "totals": {
            ...
            "awaiting_verification": sum(
                1 for lead in leads if (lead.get("verification") or {}).get("status") == "pending"
            ),
            "quiet": sum(
                1 for lead in leads if (lead.get("verification") or {}).get("status") == "quiet"
            ),
...
def list_leads(
    ...
    resolved_id, context = _require_snapshot(run_id)
    all_leads = store.lead_views(context)
    leads = all_leads
    ...
    if verification:
        leads = [
            lead
            for lead in leads
            if (lead.get("verification") or {}).get("status") == verification
        ]
    if search:
    ...
    return {
        "run_id": resolved_id,
        "count": len(leads),
        "regions": sorted({lead["region"] for lead in all_leads if lead.get("region")}),
        "weights": store.config_summary()["prospect"]["qualification_weights"],
        "status": context.get("status"),
        "quiet_count": sum(
            1 for lead in all_leads if (lead.get("verification") or {}).get("status") == "quiet"
        ),
        "pending_count": sum(
            1 for lead in all_leads if (lead.get("verification") or {}).get("status") == "pending"
        ),
        "leads": leads,
    }


@app.post("/api/leads/continue", tags=["pipeline"])
def continue_quiet_leads() -> Dict[str, Any]:
    """Resume people / video / outreach for Quiet leads only."""
    try:
        selected = store.prepare_continue()
        run = runs.start(resume=True, until=None, full=True)
    ...
    return {"selected": selected, "run": run}
...
@app.post("/api/leads/{lead_id}/verify", tags=["pipeline"])
def verify_lead(lead_id: str, payload: VerifyRequest) -> Dict[str, Any]:
    try:
        decision = store.record_verification(
            lead_id, payload.status, payload.ads_count_ui, payload.note
        )
    except FileNotFoundError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except KeyError:
        raise HTTPException(status_code=404, detail=f"No lead '{lead_id}' in the live run.")
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error

    return {"lead_id": lead_id, "verification": decision}
...
def market(run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        or (context.get("ad_patterns") or {}).get("search_terms")
        or [],
        "advertisers": advertisers,
        "ads_per_region": per_region,
        "patterns": context.get("ad_patterns") or {},
        "active_ads": [_public_ad(ad) for ad in active_ads],
    }
    ...
    try:
        return {
            "run": runs.start(
                ...
                payload.regions,
                payload.resume,
                until=None if payload.resume or payload.full else (payload.until or "prospect"),
                full=payload.full,
            )
        }
    except RuntimeError as error:
